...
async function addSkeletonToVideo2() {
    ...
    if(skeletonEnabled2) { // 스켈레톤 활성화시 pose 설정
...
function onResultsPose2(results) {
    if(skeletonEnabled2) { // 스켈레톤이 활성화되어 있을 때만 그림
        canvasCtx6.save();
        canvasCtx6.clearRect(0, 0, canvas2.width, canvas2.height);
        canvasCtx6.drawImage(
            results.image, 0, 0, canvas2.width, canvas2.height);
        drawConnectors(
            canvasCtx6, results.poseLandmarks, POSE_CONNECTIONS, {
                color: (data) => {
